...
export const Route = createFileRoute("/")({
  loader: async () => {
    const list = await getPublicEditionsList();
  ...
  head: () => ({
    meta: [
      { title: "Budapest Signal — Globális MI. Helyi lépések. Budapesti hétvégék." },
      {
        name: "description",
        content:
          "Heti jel azoknak, akik globálisan akarnak előrébb járni és helyben okosabban lépni.",
      },
    ],
  }),
  component: Home,
...
const sections = [
  {
    ...
    description:
      "A hét legfontosabb modell-, szabályozási és platform-mozgásai — leszűrve arra, ami megváltoztatja a roadmapedet.",
  },
  {
    icon: Wrench,
    label: "02",
    title: "Eszköz-radar",
    description: "Az éppen érkező eszközök, amelyek tényleg megérnek egy értékelési órát.",
  },
  {
    icon: Lightbulb,
    label: "03",
    ...
    description:
      "Egy rövid, véleményes elemzés CEE-ben építő operátoroktól — árazás, toborzás, GTM.",
  },
  {
    icon: Building2,
    label: "04",
    ...
    description:
      "Finanszírozási körök, kinevezések, nyitások és csendes lépések, amiket nem fogsz elcsípni a timelineodon.",
  },
  {
    icon: MapPin,
    label: "05",
    title: "Hétvégi jel",
    description:
      "Kurált tippek Budapest és Pest megye területéről. Hová érdemes ténylegesen elmenni.",
  },
  {
    icon: Sparkles,
    label: "06",
    ...
    description:
      "Pár cég, amiben bízunk — szerkesztői tisztelettel bemutatva, sosem szponzorált zaj.",
  },
];
...
function Home() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section
        id="subscribe"
        className="relative overflow-hidden border-b border-border/50 bg-hero-gradient grain"
      >
        ...
        <div className="relative mx-auto max-w-6xl px-5 pb-24 pt-20 md:pt-32">
          <div className="inline-flex items-center gap-2 rounded-full border border-border/60 bg-card/40 px-3 py-1 text-xs text-muted-foreground backdrop-blur">
            ...
            <span>
              {featured
                ? `Megjelent a № ${String(featured.number).padStart(3, "0")}. kiadás`
                : "Hamarosan — első kiadás"}
            </span>
          </div>

          <h1 className="mt-7 max-w-4xl font-display text-5xl leading-[1.02] tracking-tight md:text-7xl">
            Globális MI. <span className="text-muted-foreground">Helyi lépések.</span>{" "}
            <span className="text-gradient-signal">Budapesti hétvégék.</span>
          </h1>

          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-muted-foreground md:text-xl">
            Heti jel azoknak, akik globálisan akarnak előrébb járni és helyben okosabban lépni — MI,
            tech, üzlet, események és kurált hétvégi tippek Budapestről és környékéről.
          </p>
      ...
      <section className="border-b border-border/50 py-24">
        <div className="mx-auto max-w-6xl px-5">
          ...
          {featured ? (
                ...
                >
                  ...
                  <ul className="mt-5 flex flex-col divide-y divide-border/50">
                    {sec.items.slice(0, 2).map((item) => (
                      <li key={item.id ?? item.title} className="py-3 first:pt-0 last:pb-0">
            ...
            <div className="mt-10 rounded-2xl border border-dashed border-border/70 bg-muted/20 p-10 text-center">
              <p className="text-muted-foreground">
                Még nincs publikált kiadás az adatbázisban. Iratkozz fel felül — szólunk, amint
                megjelenik az első jel, vagy állítsd be a{" "}
                <code className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">
                  VITE_USE_MOCK_EDITIONS=true
                </code>{" "}
                értéket demó tartalomhoz.
              </p>
        ...
        <div className="mx-auto max-w-5xl px-5 text-center">
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-2">
            {[
              ...
              <span
                key={t}
                className="rounded-full border border-border/60 bg-card/40 px-4 py-1.5 text-sm text-foreground/80 backdrop-blur"
              >
                {t}
              </span>
            ))}
            ...
            <div className="relative grid gap-8 md:grid-cols-[1.4fr_1fr] md:items-center">
              <div>
                ...
                <p className="mt-4 max-w-xl text-muted-foreground">
                  Natív, szerkesztői színvonalú elhelyezések egy briefingben, amit az emberek
                  tényleg megnyitnak. Nincs banner, nincs csalogató — csak tisztelet az olvasó és a
                  márkád iránt.
                </p>
              </div>
              <div className="flex md:justify-end">
                <Button asChild size="lg" variant="signal">
                  <Link to="/advertise">
                    Hirdess nálunk <ArrowRight className="ml-1 h-4 w-4" />
                  </Link>
                </Button>
              </div>
            </div>
